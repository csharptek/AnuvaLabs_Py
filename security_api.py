...
# Create a new FastAPI app specifically for security scanning
security_app = FastAPI(title="Security Scanner API", version="1.0.0")
...
@security_app.get("/")
async def root():
    """Root endpoint for the Security Scanner API."""
    return {
        "message": "Security Scanner API",
        "version": "1.0.0",
        "endpoints": {
            "security_scan": "/security-testing",
            "health": "/health"
        }
    }
...
@security_app.post("/security-testing", response_model=ScanResponse)
async def security_testing(file: UploadFile = File(...)) -> ScanResponse:
    """
    Upload a ZIP file and perform comprehensive security scanning.
    
    This endpoint accepts a ZIP file containing source code and runs multiple
    security scanning tools to identify vulnerabilities. All results are
    unified into a consistent format.
    
    Args:
        file: ZIP file containing the source code to scan
        
    Returns:
        ScanResponse with scan ID, project type, and list of vulnerabilities
        
    Raises:
        HTTPException: If the file is invalid or scanning fails
    """
    # Validate file type
    ...
    try:
        ...
        if project_type == "python":
            ...
            result = run_cmd(bandit_cmd, str(extract_dir))
        ...
        # Run pip-audit for Python projects (dependency vulnerability scanner)
        if project_type == "python":
        ...
        # Run Ruff (Python linter with security rules)
        if project_type == "python":
...
if __name__ == "__main__":
    uvicorn.run("security_api:security_app", host="0.0.0.0", port=8001, reload=True)
